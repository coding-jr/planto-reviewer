...
import { ClaudeBedrockService } from './services/claudeBedrockService'
...
export const run = async (): Promise<void> => {
  ...
  const temperature = parseFloat(core.getInput('model_temperature'))

  const context = github.context
  const { owner, repo } = context.repo

  const model: BaseChatModel = new ClaudeBedrockService({
    region: awsRegion,
    modelId: modelName,
